...
def generate_signal(factor_id: str, closes: List[float],
    ...
    mapping = {
        ...
        "accdist":                   lambda: (lambda ad: [0 if i == 0 or math.isnan(ad[i]) else (1 if ad[i] > ad[i-1] else (-1 if ad[i] < ad[i-1] else 0)) for i in range(len(ad))])(accdist(highs, lows, closes, vols)),
        ...
        "mass_index":                lambda: [1 if mass > 27 else -1 for mass in mass_index(highs, lows, closes)],
        "signal_horizon":            lambda: signal_horizon(highs, lows, closes)[1],
