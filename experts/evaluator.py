...
# ── PBO 门控阈值 ───────────────────────────
PBO_HARD_REJECT   = 0.45   # PBO > 此值 → 直接 REJECT（v5: 从0.6收紧）
PBO_SOFT_DISCOUNT = 0.25   # PBO > 此值 → composite ×0.85（v5: 从0.3收紧）

# ── 打分权重 v5: 加入Alpha直接奖励 ──────────
W_SORTINO  = 0.25   # Sortino 权重
W_CALMAR   = 0.20   # Calmar 权重
W_IR       = 0.20   # IR 权重
W_DRAWDOWN = 0.20   # 回撤权重
W_ALPHA    = 0.15   # Alpha 直接奖励（策略超越基准的能力）
...
class Evaluator:
    ...
    def evaluate(self, report, template_key: str = "") -> EvalResult:
        ...
        raw_composite = (
            ...
            + ir_s      * W_IR
            + dd_s      * W_DRAWDOWN
            + max(0, alpha) * W_ALPHA  # v5: 只奖励正Alpha
        )
