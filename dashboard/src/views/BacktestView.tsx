  ...
  return (
    <div className="space-y-6">
      <div className="flex items-center gap-3 mb-4">
        ...
        <div>
          <h2 className="text-xl font-bold text-white">回测框架</h2>
          <p className="text-slate-400 text-sm">Qlib 回测引擎 · 多品种 · 多仓位</p>
        </div>
      </div>
        ...
        <div className="flex gap-4 mt-3 text-xs flex-wrap">
          {backtestResults.map(r => (
            <span key={r.year}>
              ...
              <span style={{ color: r.strategy > r.benchmark ? '#4ade80' : '#f87171' }}>
                {r.strategy > 0 ? '+' : ''}{r.strategy}%
              </span>
              <span className="text-slate-600 ml-1">
                (基准 {r.benchmark > 0 ? '+' : ''}{r.benchmark}%)
              </span>
            </span>
          ))}
        </div>
      ...
      <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
        ...
        <div className="mt-3 text-xs text-slate-500">
          等级: A ≥140分 / B ≥100分 / C ≥60分 / D &lt;60分
        </div>
      </div>

      {/* Qlib Bridge Info */}
      <div className="bg-slate-800 rounded-xl p-6 border border-slate-700">
        <h3 className="text-white font-semibold mb-3">🔗 Qlib 融合状态</h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          {[
            { label: '回测引擎',   status: '✅ Qlib SimulatorExecutor', color: '#4ade80' },
            { label: '信号输入',   status: '✅ quant orchestrator → Qlib', color: '#4ade80' },
            { label: 'Portfolio分析', status: '✅ 内置年化/夏普/回撤', color: '#4ade80' },
          ].map(item => (
            <div key={item.label} className="bg-slate-900 rounded-lg p-4 border border-slate-700">
              <div className="text-xs text-slate-500 mb-1">{item.label}</div>
              <div className="text-sm font-medium" style={{ color: item.color }}>{item.status}</div>
            </div>
          ))}
        </div>
      </div>
